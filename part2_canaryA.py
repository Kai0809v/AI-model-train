...
optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-5)  # ← 可以稍微提高初始 lr

# 【新增】使用 OneCycleLR 替代 ReduceLROnPlateau
# 策略：前 30% 的 epoch 升温到峰值，后 70% 降温
epochs = 70
steps_per_epoch = len(train_loader)

scheduler = torch.optim.lr_scheduler.OneCycleLR(
    optimizer,
    max_lr=0.002,              # 峰值学习率0.003（比初始高 3 倍）如果训练不稳定，降低 max_lr 到 0.002
    epochs=epochs,
    steps_per_epoch=steps_per_epoch,
    pct_start=0.3,             # 预热 + 上升阶段占 30%
    anneal_strategy='cos',     # 余弦退火
    cycle_momentum=False,      # 不使用动量循环（Adam 自带自适应矩估计）
    base_momentum=0.9,
    max_momentum=0.95,
    div_factor=10.0,           # 初始 lr = max_lr / div_factor = 0.0003
    final_div_factor=100.0     # 最终 lr = 初始 lr / 100 = 0.000003
)
...
for epoch in range(epochs):
    ...
    # 【移除】不再需要 scheduler.step(val_loss)
    
    if (epoch + 1) % 5 == 0 or epoch == 0:
        current_lr = scheduler.get_last_lr()[0]
        print(f"Epoch [{epoch + 1}/{epochs}] - Train Loss: {epoch_loss:.6f} - Val Loss: {val_loss:.6f} - LR: {current_lr:.6f}")
