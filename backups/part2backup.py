# train_target_denoise_transformer.py
...
test_dataset = TensorDataset(test_set_selected, test_label_raw)  # 注意这里可以随意放一个，我们稍后统一预测
...
class TransformerModel(nn.Module):
    def __init__(self, input_dim):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            batch_first=True, dropout=0.1  # 增加 dropout 防止过拟合
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=4)  # 增加层数
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.0005, weight_decay=1e-5)  # 降低学习率，增加权重衰减
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5)  # 学习率调度器
...
# 我们计算模型预测值与 "完全未经处理的真实原始功率(Raw)" 的误差，这才是最硬核的指标
...
plt.plot(range(1, len(val_losses) + 1), val_losses, marker='s', linestyle='--', color='orange', label='Val')
...
# 保存 1: Transformer 网络权重
torch.save(model.state_dict(), "../transformer_weights.pth")
print("[+] 已保存: transformer_weights.pth")

# 保存 2: LightGBM 特征选择器 (可选，若不打算在 GUI 里重新算特征重要性，仅作备份)
dump(lgb_model, "../lgb_feature_selector.joblib")
print("[+] 已保存: lgb_feature_selector.joblib")

# 保存 3: 选中的特征索引 (极度重要，GUI 推理切片必须用它！)
