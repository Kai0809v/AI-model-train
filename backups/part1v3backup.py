...
def extract_features_and_targets(train_data, test_data):
    ...
    # 新增：添加历史功率作为特征（延迟特征）
    window_hist = 12  # 增加到 12 个历史时刻
    
    def add_lag_features(data, y_raw, window_hist):
        n = len(data)
        lag_feats = []
        for lag in range(1, window_hist + 1):
            lag_col = np.roll(y_raw, lag)
            lag_col[:lag] = y_raw[lag]
            lag_feats.append(lag_col.reshape(-1, 1))
        return np.hstack([data] + lag_feats)

    X_train = add_lag_features(X_train_raw, y_train_raw, window_hist)
    X_test = add_lag_features(X_test_raw, y_test_raw, window_hist)
    ...
    X_train = add_physics_features(X_train)
    X_test = add_physics_features(X_test)
    
    print(f"特征维度：{len(weather_cols)} -> {X_train.shape[1]}")

    # Y_clean：对训练集和测试集的目标分别进行去噪提纯
    print("\n--- 训练集目标去噪 ---")
...
def normalize_data(X_train, X_test, y_train, y_test, y_test_raw):
    ...
    scaler_y = StandardScaler()  # 这个 scaler 是基于干净的 y 拟合的
