...
class SimpleMultiStepTransformer(nn.Module):
    ...
    🔧 关键改进：
    1. 移除BatchNorm（小batch不稳定）
    2. 保持4层Transformer（避免过深）
    3. 使用简单的线性预测头
    4. 添加残差连接
    """
    def __init__(self, input_dim, horizon=1):
        ...
        # 🔧 改进：更深层次的注意力池化，增强长序列的模式提取能力
        self.attention_pooling = nn.Sequential(
            nn.Linear(256, 128),
            nn.LayerNorm(128),  # 增加 LayerNorm 稳定池化分布
            nn.Tanh(),
            nn.Dropout(0.1),
            nn.Linear(128, 64),
            nn.Tanh(),
...
for h in [1, 4, 8, 16]:
    ...
    # 设置随机种子
    torch.manual_seed(42 + h)  # 不同步长用不同种子
    np.random.seed(42 + h)
    random.seed(42 + h)
    ...
    for epoch in range(epochs):
        ...
        for x, y in train_loader:
            ...
            pred = model(x)
            loss = nn.MSELoss()(pred, y)  # 使用标准MSE
...
plt.tight_layout()
plt.savefig("multi_horizon_v2_results.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.title('Multi-Horizon Prediction Performance (V2 - Independent Models)')
plt.xticks(x_pos, horizons)
plt.legend()
plt.grid(True, linestyle='--', alpha=0.6, axis='y')

plt.tight_layout()
plt.savefig("multi_horizon_v2_comparison.png", dpi=150, bbox_inches='tight')
plt.show()
